...
public class SupplierRepository : ISupplierRepository
{
    ...
    public async Task<Supplier?> Get(Guid idSupplier, CancellationToken cancellationToken)
        => await _postgresDbContext.Suppliers
            .Where(supplier => supplier.Id == idSupplier)
            .FirstOrDefaultAsync(cancellationToken);

    public async Task<Supplier?> Get(string nameSupplier, CancellationToken cancellationToken)
        => await _postgresDbContext.Suppliers
